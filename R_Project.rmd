...
- **Dynamic Visualization** By using the plotly package, users are able to interact with the visualization to compare how fencers perform on average as compared to other clubs.
...
![](C:\\Users\\Nicholas\\Pictures\\Screenshots\\Raffles invites screenshot.png)  
...
Lastly, I presented the summary generated with a dynamic visualisation using plotly and as tabular format on Shiny for easy viewing.
...
## Display the processed data in a table
  ...
  ggplot(data = Final_Data(), aes(x = Club_Name, y = Average_Position)) +
  geom_bar(fill="deepskyblue",stat="identity")+
  labs(title = "Fencer Statistics by Club",
       x = "Total Number of Fencers",
       y = "Average Position") +
...
- **The ggplot2 and plotly package** allows me to create easy to view dynamic visualizations and an alternative way for users to view the computed summary.
